...
export default function ProfessionalTimeline({
  ...
  // Enhanced role classification - MOVED UP BEFORE getSortedRoles
  const getRoleMode = (role) => {
    if (role.tags?.includes("PM") && role.tags?.includes("DA")) {
      return "both";
    }
    if (role.tags?.includes("DA")) {
      return "data-analyst";
    }
  ...
  // Create sorted array based on mode - FIXED
  const getSortedRoles = () => {
    // Default order: newest first (Master Thesis at top) - FIX FOR ISSUE 1
    const defaultOrder = timelineRoles.slice()//.reverse();
    
    // If no mode is selected or highlight is false, use default order
    if (!highlight || !mode) {
      return defaultOrder;
    }
    
    const rolesCopy = [...timelineRoles];
    
    if (mode === "data-analyst") {
      // Data Analyst first, then Process Mining (but maintain relative chronological order within each group)
      const dataAnalystRoles = rolesCopy.filter(role => {
        const mode = getRoleMode(role);
          return mode === "data-analyst" || mode === "both";
      });
      const processMiningRoles = rolesCopy.filter(role => {
        const mode = getRoleMode(role);
        return mode === "process-mining";
      });
      
      // Sort each group chronologically (newest first within each group)
      dataAnalystRoles.sort((a, b) => {
        const aIndex = timelineRoles.findIndex(r => r.id === a.id);
        const bIndex = timelineRoles.findIndex(r => r.id === b.id);
        return aIndex - bIndex; // Reverse chronological (newest first)
      });
      
      processMiningRoles.sort((a, b) => {
        const aIndex = timelineRoles.findIndex(r => r.id === a.id);
        const bIndex = timelineRoles.findIndex(r => r.id === b.id);
        return aIndex - bIndex; // Reverse chronological (newest first)
      });
      
      return [...dataAnalystRoles, ...processMiningRoles];
      
    } else if (mode === "process-mining-analyst" || mode === "process-mining") {
      // Process Mining first, then Data Analyst (but maintain relative chronological order within each group)
      const processMiningRoles = rolesCopy.filter(role => {
        const mode = getRoleMode(role);
        return mode === "process-mining" || mode === "both";
      });
      const dataAnalystRoles = rolesCopy.filter(role => {
        const mode = getRoleMode(role);
        return mode === "data-analyst";
      });
      ...
      return [...processMiningRoles, ...dataAnalystRoles];
    }
  ...
  useEffect(() => {
    ...
    const calculatedWidth = (sortedRoles.length * nodeWidth) + ((sortedRoles.length - 1) * gap) + (padding * 2);
  ...
  const getRoleVisibility = (role) => {
    ...
    if (isDA && (roleMode === "data-analyst" || roleMode === "both")) return "glow";
    if (isPM && (roleMode === "process-mining" || roleMode === "both")) return "glow";
    return "dimmed";
  };
  ...
  const handleNodeClick = (role, displayIndex) => {
  
  setShowTataSteel(false);
  const originalIndex = timelineRoles.findIndex(r => r.id === role.id);
        ...
        <div className="timeline-nodes">
          {sortedRoles.map((role, displayIndex) => {
            ...
            return (
              <div
                key={role.id}
                className={`timeline-node ${visibility} ${roleMode} ${selected === timelineRoles.findIndex(r => r.id === role.id) ? 'selected' : ''}`}
                onClick={() => handleNodeClick(role, displayIndex)}
                style={{
              ...
              >
                ...
                <div className="timeline-node-content">
                  ...
                  <div className="timeline-skills">
                    {role.tags?.includes("DA") && (
                      <span className="skill-indicator da">DA</span>
                    )}
                    {role.tags?.includes("PM") && (
                      <span className="skill-indicator pm">PM</span>
                    )}
                  </div>
                </div>
                ...
                {displayIndex < sortedRoles.length - 1 && (
